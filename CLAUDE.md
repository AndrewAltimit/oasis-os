...
OASIS_OS is an embeddable operating system framework in Rust (edition 2024). It provides a skinnable shell with a scene-graph UI, command interpreter, virtual file system, browser engine (HTML/CSS/Gemini), plugin system, and remote terminal. It renders to any pixel buffer + input stream. Built from scratch in Rust starting early 2026, inspired by PSP homebrew shells like PSIX. Twenty skins are implemented (14 external TOML skins, 20 built-in; external skins also have built-in equivalents).
...
- **oasis-skin** -- Data-driven TOML skin system with 20 skins (14 external TOML in `skins/`, 20 built-in). Theme derivation from 9 base colors.
