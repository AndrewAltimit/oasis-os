...
impl PspVideoDecoder {
    ...
    fn try_init() -> Result<Self, String> {
        ...
        if ret < 0 {
            // SAFETY: Release the codec handle opened by sceVideocodecOpen
            // above. Without this, the codec slot leaks on repeated failures.
            unsafe { psp::sys::sceVideocodecStop(ptr, 0) };
