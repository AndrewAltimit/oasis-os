...
impl SdlBackend {
    ...
    pub(crate) fn render_glyph_texture(
        ...
        // The glyph coverage is shared with the software rasterizer (UE5
        // framebuffer, e2e harness), so both draw identical text.
        let mut mask = Vec::new();
        let (gw, gh) = oasis_rasterize::glyph_mask(ch, font_size, bold, italic, &mut mask);
        let mut rgba = vec![0u8; (gw * gh * 4) as usize];
        for (px, &ink) in rgba.as_chunks_mut::<4>().0.iter_mut().zip(&mask) {
            if ink {
                *px = [color.r, color.g, color.b, color.a];
            }
        }
...
impl SdiText for SdlBackend {
    ...
    fn measure_text_height(&self, font_size: u16) -> u32 {
        ...
        // Shared with the software rasterizer and WASM.
