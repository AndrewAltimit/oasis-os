...
use super::super::parser::{Rule, SimpleSelector, Stylesheet};
...
pub struct SelectorIndex {
    ...
    /// The rules with at least one pseudo-element selector (`p::before`),
    /// bucketed the same way. `::before` / `::after` resolution runs for
    /// every element; without this it tested every rule of every sheet
    /// (most of the cascade time on Wikipedia).
    pseudo_element: Option<Box<SelectorIndex>>,
}
...
impl SelectorIndex {
    ...
    pub fn build(stylesheets: &[&Stylesheet]) -> Self {
        ...
        for (sheet_idx, sheet) in stylesheets.iter().enumerate() {
            for (rule_idx, rule) in sheet.rules.iter().enumerate() {
                ...
                index.file(entry, rule);
                if rule
                    .selectors
                    .selectors
                    .iter()
                    .any(|s| super::matching::selector_pseudo_element(s).is_some())
                {
                    pseudo.file(entry, rule);
                }
            }
        }

        index.pseudo_element = Some(Box::new(pseudo));
        index
    }

    /// File `entry` (for `rule`) under the buckets of its selectors'
    /// subjects.
    fn file(&mut self, entry: IndexedRule, rule: &Rule) {
                ...
                for simple in &subject.0.parts {
                    match simple {
                        ...
                        SimpleSelector::Class(cls) => {
                            self.by_class.entry(cls.clone()).or_default().push(entry);
                            selector_filed = true;
                        },
    ...
    /// Candidate rules with a pseudo-element selector that might apply
    /// to an element with the given tag, id and classes, in source order.
    pub(super) fn pseudo_element_candidates(
        ...
        self.pseudo_element.as_ref().map_or_else(Vec::new, |p| {
            p.candidates_with_lower(tag, tag_lower, id, classes)
        })
    }
