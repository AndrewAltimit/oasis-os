...
/// because OASIS OS has its own music player (sceMp3) which shares the
/// ME coprocessor's EDRAM with sceAudiocodec. Both cannot be active
/// simultaneously.
///
/// Detection: scan the first 1MB of user memory for the "OASIS_OS"
...
unsafe fn is_oasis_running() -> bool {
    ...
    while addr <= end - (needle.len() as u32) {
        ...
        while j < needle.len() {
            let byte = unsafe {
                core::ptr::read_volatile((addr + j as u32) as *const u8)
            };
            if byte != needle[j] {
                matched = false;
                break;
    ...
    // so we check multiple times with increasing delays to catch it.
    {
        ...
        for (i, &delay) in delays.iter().enumerate() {
            ...
            if unsafe { is_oasis_running() } {
                crate::debug_log(
                    b"[OASIS] OASIS_OS detected, skipping PRX audio",
                );
                return 0;
            }
            if i < delays.len() - 1 {
