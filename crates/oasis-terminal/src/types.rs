...
pub enum CommandSignal {
    ...
    /// Inspect the host's SDI scene graph (`sdi list` / `sdi get <name>`).
    ///
    /// Commands have no access to the scene registry, so the host resolves
    /// this into text (see `oasis_core::terminal_sdi::inspect_sdi`).
    SdiInspect {
        /// Object to describe; `None` lists every object.
        name: Option<String>,
    },
...
/// Maximum number of history entries to retain (in memory and in the
